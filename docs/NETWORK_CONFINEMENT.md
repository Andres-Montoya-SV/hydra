# Hydra — Network Confinement

**Status: living reference.** Originally written 2026-08-30/31 as
`FINAL_NETWORK_CONFINEMENT_AUDIT.md` (a point-in-time forensic audit,
preserved in full at `docs/archive/FINAL_NETWORK_CONFINEMENT_AUDIT_2026-08-31.md`);
carried forward here as the standing reference for how Hydra confines
outbound network traffic to authorized targets. It replaces three prior,
overlapping documents (`docs/archive/NETWORK_BOUNDARY_AUDIT.md`,
`docs/archive/FINAL_NETWORK_BOUNDARY_AUDIT.md`, and the original of this
file) — read those only for the historical narrative of how each fix in
this document was found and closed.

**Re-verified live, 2026-08-30 → 2026-09-13**: `docs/FINAL_PROJECT_AUDIT.md`
independently re-ran the confinement live-test suite this document's
evidence rests on (22/22 passed — real local arbiter server, real
`httpx`/`katana`/`hakrawler`/urllib-plugin invocations, zero unauthorized
connections) and found no drift. One correction confirmed since the
original text below was written: **WHOIS is no longer the unconfined case
Section D's own honest-limitations framing once implied** —
`modules/whois.py` now uses a native, SSRF-validated client
(`core/collection/whois_client.py`, Class B* throughout this document),
not the raw system `whois` binary. Everything else below was re-confirmed
accurate as of 2026-09-13.

You will see "this turn"/"this round" throughout the body below — that
phrasing is preserved verbatim from the original forensic-audit sessions
that produced each section; read it as "at the time this section was
written," not as anything recent. New findings should be added as new
dated sections at the end, not by editing the historical narrative in
place.
