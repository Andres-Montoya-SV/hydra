...
- Dependencies: upgrade python-dotenv from 1.0.1 to patched 1.2.2 in both
  ...
  and weekly dependency/action update PRs. Also patch Black to 26.3.1 and pytest
  to 9.0.3, with pytest-asyncio 1.3.0 for pytest 9 compatibility. Audit all Python
  requirement sets installed in Docker, not only API dependencies. This does
  not audit the separate Go binaries or operating-system packages.
...
## Optional TLS capability security hold

The complete Docker Python audit also exposed cryptography 46.0.7 through
SSLyze 6.3.1's `<47` requirement. The supported resolver cannot install it
alongside the corrected cryptography 50.0.1. SSLyze is therefore excluded from
the default image and both execution and automatic installation are blocked.
Archived-output parsing remains available. See
[SSLyze security hold](SSLYZE_SECURITY_HOLD.md) for the advisory evidence,
regression tests and conditions for restoring this optional capability.
No audit exceptions or dependency-resolution overrides were added. The corrected
complete Python requirement audit reports no known vulnerabilities. Security-hold,
installer-policy and toolchain consistency regressions: 21 passed locally.
