...
## 9. Crawler confinement, `AuthorizedCollectionTarget`, and further fixes (this change set)

The two items §8 explicitly deferred — the crawler containment proxy and a typed authorization proof — are built in this pass, along with several findings surfaced by writing real, adversarial tests rather than assuming existing coverage was sufficient. Full detail and the exact runtime/collection/authorization graphs are in `docs/FINAL_SECURITY_AUDIT.md`; this section is the changelog.

1. **Crawler containment proxy (`core/collection/crawler_proxy.py`, closes the CRITICAL finding in §6).** `ScopeEnforcingProxy` is a local HTTP/HTTPS forward proxy: it authorizes the destination host against `CollectionScope` *before* connecting anywhere, for both `CONNECT` (HTTPS — a pure TCP splice after authorizing the tunnel target, no TLS interception) and plain HTTP (absolute-URI proxy requests). katana, hakrawler, and nuclei are now **always** launched with `-proxy <local-proxy-url>` (`modules/_base.py:_crawler_confinement`, wired into all three plugins) — not just under `STRICT_OPSEC`. nuclei additionally gets `-ni` by default (`Settings.nuclei_enable_interactsh`, default off) because interactsh OOB polling contacts ProjectDiscovery's own public servers directly, which the confinement proxy would otherwise (correctly, but confusingly) block, silently disabling every OOB template.

   Verified three ways: (a) `tests/test_crawler_proxy.py` drives the proxy with raw sockets speaking the proxy protocol directly and asserts on whether a destination test server *ever received a TCP connection* — for CONNECT, plain-HTTP, a missing scope, and an authorization exception (fail closed) — including a POST-with-body test proving the splice-based forwarder doesn't truncate request bodies. (b) `tests/test_crawler_confinement_live.py` runs the **real installed katana and hakrawler binaries** (not mocked) against a local server that 302s to a second local server, and proves the redirect target's connection counter stays at zero. (c) Running katana live surfaced a real, unrelated environment issue (this sandbox's `~/.config` is owned by a different user, and katana refuses to start at all without write access to it) and running hakrawler live surfaced that **the installed hakrawler build had never actually worked** — `modules/hakrawler.py` was passing `-plain`/`-depth`, neither of which exist on the currently-packaged binary (`-d` for depth; plain-text is the default with no flag) — masked because every prior test mocked `_execute` and never invoked the real binary. Both are fixed (the plugin's flags; the live test sets `HOME` to a scratch directory for katana specifically).

   Explicitly not claimed: this is not TLS interception (a CONNECT tunnel is authorized by its target host, never decrypted) and not a guarantee against a tool that ignores its own `-proxy` setting via a raw-socket path outside its configured HTTP client — see `docs/FINAL_SECURITY_AUDIT.md` for the exact boundary.

2. **`AuthorizedCollectionTarget` (`core/collection/target.py`, closes the MEDIUM finding "no structural constraint" in §6, partially).** A frozen dataclass constructible only via `AuthorizedCollectionTarget.authorize(raw, scope, capability=...)`, which returns `None` on DENY/UNKNOWN and a populated, immutable target on ALLOW — there is no other way to obtain one. Wired into the highest-rigor existing call site, `modules/httpx.py`'s redirect-hop resolution: `_fetch_single_hop` now takes the target object itself, not a URL string, so the function signature enforces that its caller went through authorization first. This does not retrofit every plugin (that is the full `CollectionGateway`, still deferred) — it demonstrates the pattern at one real site with a passing test suite (`tests/test_authorized_collection_target.py`) rather than leaving it as an unused type nobody calls.

3. **Historical diff falsely reports every relationship as changed on every re-scan (new finding, fixed, `core/diff.py`).** Found while writing the tests the mission's Section 18 asked for (same relationship_id, confidence HIGH→MEDIUM; same relationship_id, new evidence). `_intel_relationship_diff` compared `evidence_id` directly — but `evidence_id` is derived from an `observation_id` that embeds `run_id` (`core/intel/engine.py:_observe`), so it differs between *any* two runs even when nothing at all changed. Two identical back-to-back scans would have shown 100% of relationships as `EVIDENCE_CHANGED`, making the feature useless for spotting real change. Fixed to compare the relationship's own `data` content (minus the `observed_at` timestamp key) instead. `tests/test_intel_diff.py` now includes both scenarios the mission specified, plus a third proving a byte-identical re-scan reports zero changes — the case that would have caught this bug immediately.

4. **`cmd_investigate` didn't use the canonical relationship serializer (new finding, fixed, `core/intel/query.py`).** `cmd_relationships` and `core/reporter.py` both already routed through `core.intel.serialize.serialize_relationship` (§17's requirement was largely already met), but `IntelQuery.investigate()` — what `cmd_investigate` prints — returned raw, unserialized DB rows for its `relationships` key, a second, structurally different shape for the same underlying data. Fixed to call `serialize_relationships(...)` with the same evidence/entity context already being built for the explanations list. `tests/test_intel_cli.py::test_investigate_and_relationships_agree_on_the_same_relationship` asserts `cmd_investigate` and `cmd_relationships` report identical `confidence_band`/`certificate_fingerprint`/`relationship_type` for the same relationship_id.
