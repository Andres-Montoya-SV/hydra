...
| **amass** | **Exact pin to v4, enforced by a runtime compatibility gate** | Homebrew's `amass` formula (and `go install ...@master`/`@latest`) tracks upstream v5.x, whose CLI replaced the single-process `enum -o <file>` model this plugin depends on with a client/server architecture (`amass engine` + `amass enum` as a client + a separate `amass subs` query step against a graph database) — confirmed by direct testing (a real v5.1.1 run against a real domain returned zero results even with a 2-minute timeout), not just the missing `-o` flag. That is a different, larger integration than this plugin implements, so v4 is the supported version: `go install -v github.com/owasp-amass/amass/v4/...@v4.2.0` — **there is no `amass@4`-style versioned Homebrew formula**, so `go install` pinned to this exact tag is the only reliable way to get v4 on any platform, including macOS. `python app.py check-tools` still detects and refuses a v5.x install with this exact fix (`core/dependencies/registry.py::KNOWN_INCOMPATIBLE_VERSIONS`) before the plugin ever attempts to run, in case v4 isn't what ends up installed. Separately, `modules/amass.py` also had to be fixed to parse v4's real `-o` output — a relationship-transcript format (`"host (FQDN) --> record_type --> target (Type)"`), not the plain subdomain-per-line list the plugin previously assumed — verified against a real v4.2.0 run, not assumed from older documentation. See `docs/HARDENING_ROUND2_P1.md` (Task 1, the detection gate) and this repo's amass cleanup commit (the v4 pin + parser fix) for the full history. |
...
| **sslyze**, **wafw00f** | **Exact pin, `pip install -r requirements-optional.txt`** | Pure Python, no separate binary to discover — installs into the same venv as `python-docx`/`playwright`. Already in the Docker image's final stage for free (it already runs `pip install ... -r requirements-optional.txt`); no Dockerfile change needed. Versions (`sslyze==6.3.1`, `wafw00f==2.4.2`) confirmed installed and run for real against `example.com` during `modules/sslyze.py`/`modules/wafw00f.py`'s own development. |
| **theHarvester** | **Manual — real, tested locally; NOT added to the Docker image in this task** | Requires Python **>=3.14**, managed by `uv`, cloned from source (confirmed: not on PyPI, not a `go install` binary). The real setup — `git clone https://github.com/laramies/theHarvester.git && cd theHarvester && uv sync` — was actually run end-to-end during `modules/theharvester.py`'s own development (`uv` downloaded and managed its own Python 3.14 automatically, no system Python 3.14 needed) and produced real, working output. **Stated honestly rather than guessed**: this task had no running Docker daemon available to build and verify a Dockerfile stage for it, and theHarvester's atypical Python-version/uv requirement is exactly the kind of thing this project's own discipline says to verify before shipping, not assume — so it was deliberately left OUT of the Dockerfile rather than adding an unverified multi-stage build. `ENABLE_THEHARVESTER` defaults to `false`; enabling it today means installing it manually on whatever host actually runs the container/native process, pointing `THEHARVESTER_PATH` at a wrapper script (`uv run --directory <clone> theHarvester "$@"`, or the clone's own `.venv/bin/theHarvester` console script directly once `uv sync` has run). A real Docker stage for this is a reasonable follow-up once someone can actually build and test one. |
| **gitleaks** | **`go install`, real gotcha found; NOT added to the Docker image in this task** | Confirmed by actually running `go install` (not assumed): the project moved from the `zricethezav` GitHub user to the `gitleaks` org, but its `go.mod` still declares the OLD module path — `go install github.com/gitleaks/gitleaks/v8@...` fails with a real "version constraints conflict" error; the correct, current working install is `go install github.com/zricethezav/gitleaks/v8@v8.30.1` (confirmed installed and run for real, including a real `go install` end-to-end, during `modules/github_secrets.py`'s own development — also installable via `brew install gitleaks`, confirmed 8.30.1). This follows the exact same `go install`-into-`GOBIN` pattern this Dockerfile already uses successfully for 5 other tools (subfinder/dnsx/httpx/naabu/katana/nuclei), so the install mechanism itself carries low risk — but this task had no running Docker daemon available to actually build and verify adding a 6th line to that same block, so it was left out rather than assumed to work. `ENABLE_GITHUB_SECRETS` defaults to `false`; enabling it today means installing `gitleaks` natively (`brew install gitleaks` or the `go install` command above) wherever the container/native process runs. Adding the Dockerfile line is a low-risk, real follow-up once someone can build and test it. |
